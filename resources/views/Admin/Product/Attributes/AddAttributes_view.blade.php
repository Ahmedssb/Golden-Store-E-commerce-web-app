					...
					<div class="card-body">   
						...
						<table id="bootstrap-data-table-export" class="table table-striped table-bordered">
							...
							<tbody>
							 ...
							 <tr  >
							 <form action="{{route('ProductAttributes.Update')}}" method="post"   enctype="multipart/form-data"     >
							 {{csrf_field()}}
									<td><input type="hidden" name="attId" value="{{$att->id}}">{{$att->id}}</td>
									<td>{{$att->sku}}</td>
									<td>{{$att->size}}</td>
									<td><input type="text" name="attPrice" value="{{$att->price}}"></td>
									<td><input type="text"  name="attStock" value="{{$att->stock}}"></td>
									 
									<td> 
								  <input type="submit" value="update" class="btn btn-primary"  id="{{$att->price}}">  
								  <a class="btn btn-danger btn-sm" href=" # " id="delete_cat"><button id="{{$att->id}}" type="button" onclick="showconfirmation('{{$att->id}}','{{route('ProductAttributes.Delete',['id'=>$att->id])}}')"> Delete</button></a>
